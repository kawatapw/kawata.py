...
def _find_state_in_artifacts(
    ...
    print("DEBUG: No matching state found")
    return {}
...
def generate(
    ...
    print(
        f"DEBUG: generate() called with workflow_name={workflow_name}, run_id={run_id}",
    )
    ...
    if state:
        print("DEBUG: State found, updating template data")
        print(
            f"DEBUG: State status={state.get('status')}, duration={state.get('duration')}",
        )
        ...
        print("DEBUG: No state found, using default template data")
    ...
    if args.artifact_dir:
        ...
        if artifact_path.exists():
            ...
            print(
                f"DEBUG: Artifact results: test_results={len(artifact_results.get('test_results', ''))}, lint_results={len(artifact_results.get('lint_results', ''))}, security_results={len(artifact_results.get('security_results', ''))}",
            )
        else:
...
def parse_artifacts_structured(artifact_path: Path) -> dict[str, Any]:
    ...
    for category, config in report_categories.items():
        ...
        for f in artifact_path.rglob("*"):
            ...
            matches_file = any(
                expected in filename for expected in config_dict["files"]
            )

            # Check if file matches parser type in filename
            matches_parser = any(
                parser in filename for parser in config_dict["parsers"]
            )
            ...
            if matches_file or matches_parser:
                try:
                    ...
                    print(
                        f"DEBUG: Parsing {f.name} with parser {parser_type} for category {category}",
                    )
    ...
    if critical > 0 or high > 0:
        ...
        if critical_vulns:
            ...
            for v in critical_vulns[:5]:  # Limit to first 5
                ...
                output += (
                    f"- {v.get('id', '')}: {pkg_info} - {v.get('title', '')[:80]}\n"
                )
