...
async def initialize_housekeeping_tasks() -> None:
    ...
    log(f"Successfully initialized {len(app.state.sessions.housekeeping_tasks)} housekeeping tasks", Ansi.LGREEN)
    ...
    while True:
        try:
            ...
            if seasons_enabled != '1':
                log("Seasons not enabled, skipping check", Ansi.LYELLOW, level=logLevel.DEBUG)
                await asyncio.sleep(interval)
                continue
            ...
            schedules: list[seasons_repo.SeasonSchedule] = await seasons_repo.fetch_active_schedules()
            ...
            for schedule in schedules:
                log(f"Processing schedule: {schedule['name']} (ID: {schedule['id']}, Type: {schedule['schedule_type']})", 
                    Ansi.LCYAN, level=logLevel.DEBUG)
                ...
                log(f"Using provider for schedule type: {schedule['schedule_type']}", 
                    Ansi.LCYAN, level=logLevel.DEBUG)
                ...
                log(f"Found {len(existing_seasons)} existing season(s) for schedule '{schedule['name']}'", 
                    Ansi.LCYAN, level=logLevel.DEBUG)
                ...
                if active_season is None:
                    log(f"No active season found for schedule '{schedule['name']}'", 
                        Ansi.LYELLOW, level=logLevel.DEBUG)
                    ...
                    for season in existing_seasons:
                        ...
                        log(f"Checking season '{season['name']}': {start_date} to {end_date}", 
                            Ansi.LCYAN, level=logLevel.DEBUG)
                        
                        if start_date <= current_time < end_date:
                            current_season = season
                            log(f"Found current season: {season['name']}", 
                                Ansi.LCYAN, level=logLevel.DEBUG)
                            break
                    
                    if current_season:
                        # Activate the current season
                        log(f"Activating season: {current_season['name']} (ID: {current_season['id']})", 
                            Ansi.LCYAN)
                        ...
                        log(f"No season covers current time for schedule '{schedule['name']}', creating new season", 
                            Ansi.LCYAN)
                        ...
                        log(f"Creating new season: {season_name} ({start_date} to {end_date})", 
                            Ansi.LCYAN)
                    ...
                    log(f"Found active season: {active_season['name']} (ID: {active_season['id']})", 
                        Ansi.LCYAN, level=logLevel.DEBUG)
                    ...
                    log(f"Active season ends at: {end_date}, current time: {current_time}", 
                        Ansi.LCYAN, level=logLevel.DEBUG)
                    
                    if end_date <= current_time:
                        log(f"Active season '{active_season['name']}' has ended, deactivating", 
                            Ansi.LMAGENTA)
                        ...
                        log(f"Creating next season: {season_name} ({start_date} to {end_date})", 
                            Ansi.LCYAN)
                        ...
                        log(f"Active season '{active_season['name']}' is still active", 
                            Ansi.LCYAN, level=logLevel.DEBUG)
        ...
        log(f"Season check complete, sleeping for {interval} seconds", 
            Ansi.LCYAN, level=logLevel.DEBUG)
        await asyncio.sleep(interval)
...
async def _retroactively_create_seasons(
    ...
    try:
        ...
        if oldest_play_time.tzinfo is None:
            oldest_play_time = oldest_play_time.replace(tzinfo=timezone.utc)
...
async def calculate_season_stats_for_all_users(season_id: int) -> None:
    ...
    try:
        ...
        users: list[dict[str, Any]] = await app.state.services.database.fetch_all(
        ...
        await stats_repo.ensure_season_rows_for_users(season_id=season_id, user_ids=user_ids)
        ...
        for user in users:
            for mode in GameMode:
                try:
                    ...
                    log(f"Failed to calculate stats for user {user['userid']}: {e}",
                        Ansi.LRED, level=logLevel.ERROR)
